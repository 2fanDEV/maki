...
impl Classifier for NearestCentroid {
    fn train(x: &CsMat<f64>, y: &[ObjectId]) -> Result<Self> {
        ...
        let classes: Vec<_> = y
            .iter()
            .map(|label| labels.binary_search(label).unwrap())
            .collect();
...
mod tests {
    ...
    fn learns_class_means_and_predicts_with_either_sparse_layout(#[case] csc: bool) {
        ...
        let model =
            NearestCentroid::train(&matrix, &[label(3), label(3), label(8), label(8)]).unwrap();

        assert_eq!(model.labels(), vec![label(3), label(8)]);
        ...
        assert_eq!(
            ...
            [label(3), label(8), label(3), label(8), label(3)]
        );
    ...
    fn averages_implicit_zeros_and_accepts_a_single_class() {
        ...
        let model = NearestCentroid::train(&matrix, &[label(5); 3]).unwrap();
        assert_eq!(model.centroids, [vec![2.0]]);
        let features = CsMat::new((1, 1), vec![0, 1], vec![0], vec![100.0]);
        assert_eq!(model.predict(&features), [label(5)]);
    }

    #[test]
    fn rejects_invalid_training_data() {
        assert!(NearestCentroid::train(&CsMat::zero((0, 2)), &[]).is_err());
        assert!(NearestCentroid::train(&CsMat::zero((1, 0)), &[label(0)]).is_err());
        assert!(NearestCentroid::train(&training_data(), &[label(1)]).is_err());
        for value in [f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
            let matrix = CsMat::new((1, 1), vec![0, 1], vec![0], vec![value]);
            assert!(NearestCentroid::train(&matrix, &[label(1)]).is_err());
        }
    }
    ...
    fn rejects_wrong_prediction_dimensions() {
        let model = NearestCentroid::train(&training_data(), &[label(1); 4]).unwrap();
